...
@onready
var file_dialog_use_script: FileDialog = $"Import/Margins/Scroll/Box/Grid/HBoxContainer/FileDialog"
...
func _import_settings_from_settings_file(
...
func _create_new_settings_file(textfile_path: String):
	...
	if script_path_field.text:
		...
		var uniques := {}
		import_data.prop_types = ResourceTablesImport.get_resource_property_types(
			existing_resource, import_data.prop_names, uniques
		)
...
func _create_prop_editors():
	...
	for i in import_data.prop_names.size():
		...
		new_node.display(
			...
			prop_type if !(prop_type is PackedStringArray) else ResourceTablesImport.PropType.ENUM
		)
...
func _on_export_csv_pressed():
	...
	var column_properties: Dictionary = editor_view.node_columns.column_properties[
		editor_view.current_path
	]

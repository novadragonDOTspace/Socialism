...
@onready
var node_table_root: Container = $"HeaderContentSplit/MarginContainer/FooterContentSplit/Panel/Scroll/MarginContainer/TableGrid"
...
func display_folder(
	...
	if (
		(folderpath.ends_with(".tres") or folderpath.ends_with(".res"))
		and !(load(folderpath) is ResourceTablesImport)
	):
...
func _update_row(row_index: int, color_rows: bool = true):
	...
	var shortened_path: String = (
		rows[row_index]
		. resource_path
		. substr(current_path.length())
		. trim_suffix(".tres")
		. trim_suffix(".res")
	)

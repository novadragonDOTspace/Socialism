...
func set_value(node: Control, value):
	...
	var cell_label_mode: int = ProjectSettings.get_setting(
		TablesPluginSettingsClass.PREFIX + "resource_cell_label_mode", 0
	)
	...
	for i in children.size():
		if i >= value.size():
			...
			_write_value_to_child(
				value[i], value[i], column_hints, children[i], color_tint, cell_label_mode
			)


func _write_value_to_child(
	value, key, hint_arr: PackedStringArray, child: Label, color_tint: float, cell_label_mode: int
):
...
static func _resource_to_string(res: Resource, cell_label_mode: int):
	...
	return (
		prefix
		+ (res.resource_name if res.resource_name != "" else "[%s]" % res.resource_path.get_file())
	)

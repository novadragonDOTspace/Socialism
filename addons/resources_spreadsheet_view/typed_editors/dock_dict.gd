...
func _add_value(value):
	...
	var cur_value
	var dupe_value: bool = ProjectSettings.get_setting(
	...
	for i in values.size():
		...
		if (
			dupe_value
			and (_stored_type == TYPE_DICTIONARY or cur_value.resource_path.rfind("::") != -1)
		):
	...
	if value is Dictionary:
		...
		for i in lines.size():
			if values[i] is Array or values[i] is Dictionary:
				lines[i] = (
					"%s%s : %s" % [cur_indent, var_to_str(keys[i]), var_to_str_no_sort(values[i])]
				)

...
func try_edit_value(value, type, property_hint) -> bool:
	...
	button_box.get_child(1).visible = (
		is_generic_array
		or value.get_typed_builtin() == TYPE_STRING
		or value.get_typed_builtin() == TYPE_STRING_NAME
		or _stored_type == TYPE_PACKED_STRING_ARRAY
	)
	button_box.get_child(2).visible = (
		is_generic_array
		or value.get_typed_builtin() == TYPE_INT
		or _stored_type == TYPE_PACKED_INT32_ARRAY
		or _stored_type == TYPE_PACKED_INT64_ARRAY
	)
	button_box.get_child(3).visible = (
		is_generic_array
		or value.get_typed_builtin() == TYPE_FLOAT
		or _stored_type == TYPE_PACKED_FLOAT32_ARRAY
		or _stored_type == TYPE_PACKED_FLOAT64_ARRAY
	)
	...
	if value.get_typed_builtin() == TYPE_OBJECT:
		if !value_input is EditorResourcePicker:
			var new_input: EditorResourcePicker = (
				load("res://addons/resources_spreadsheet_view/editor_resource_array_picker.gd")
				. new()
			)
...
func _add_value(value):
	...
	var cur_value
	var dupe_array: bool = ProjectSettings.get_setting(
		TablesPluginSettingsClass.PREFIX + "dupe_arrays"
	)
...
func _add_recent(value):
	...
	if value is Resource:
		value_str = (
			value.resource_path.get_file() if value.resource_name == "" else value.resource_name
		)
